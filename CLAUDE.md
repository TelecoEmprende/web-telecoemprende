...
- Frontend tests use Vitest + Testing Library (`src/test/setup.ts`), colocated as `*.test.tsx` next to the component (see `RegistrationForm.test.tsx`, `routes/admin/AdminPanel.test.tsx`). Backend tests are a single `unittest` suite in `tests/test_api.py`.
