...
- **The `/equipo` self-serve workspace** is a separate system from `/admin`, with its own email+password login (`equipo_api` in `backend/api/equipo.py`, `services/equipo.py`, table `equipo_accesos`) — not the same auth as `ADMIN_PASSWORD`. A person's `equipos` array (`marketing`/`eventos`/`ingenieria`) and `cargo` (`presidente`/`boardmember`/``) decide what they see; anyone with `ingenieria` or a board `cargo` also gets an admin session and can reach `/admin` (`EQUIPO_CON_PERMISOS_ADMIN` in `backend/config.py`). Accounts, department membership, and the shared calendar are managed from `/admin` (`EquipoAccesosPanel`, `CalendarioPanel`) — `/equipo` itself only reads them (`GET /api/equipo/session`, `GET /api/equipo/calendario`).
  ...
  - **El movimiento del CRM (`/equipo` y `/admin`) sale todo de `frontend/src/components/movimiento.tsx`**, construido sobre las constantes de [SmoothUI](https://smoothui.dev) (`components/smoothui/lib/animation.ts`, instalado con `npx shadcn@latest add @smoothui/<nombre>` — el namespace `@smoothui` ya está en `components.json`). Exporta `PanelAnimado` (entrada al cambiar de sección), `FilaAnimada`/`useEntradaDeFila` (entrada escalonada + recolocación de tarjetas, filas y `<tr>`) y `Contador` (números que ruedan al cambiar). Reglas al añadir animaciones nuevas: solo `transform` y `opacity`, 0.2-0.3 s, muelle `bounce` 0.1, y `useReducedMotion` siempre respetado. Dos avisos que ya costaron un rato: `motion.button` se queda con `onDragStart`/`onDragEnd` para su propio sistema de arrastre, así que lo que use drag nativo (el tablero de `TasksPanel`) se envuelve en un `motion.div` en vez de convertirse; y una barra de progreso no puede llevar a la vez `transition: transform` en CSS y una animación de motion — las dos capas se pelean y el avance se queda a medias. Los componentes de SmoothUI que traen `tokens.json` (`animated-tabs`, `notification-badge`) meten en `index.css` una paleta rosa + grises propia que no es la de marca: tras instalarlos hay que revertir `index.css` y repuntar sus clases a `tokens.css`.
